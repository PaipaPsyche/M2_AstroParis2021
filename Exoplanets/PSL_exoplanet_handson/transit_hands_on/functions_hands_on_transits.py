...
def plot_best_fit(x, y, yerr, soln, mask=None):
    ...
    gp_mod = soln["gp_pred"] + soln["mean"]
    ax.plot(
        x[mask], gp_mod+1+mod, color="red", label="without transit", zorder=45, lw=1
    )
    ax.plot(
        x[mask], gp_mod+1,'--', color="C2", label="best fit transit model", zorder=41, lw=1
    )
    ax.legend(fontsize=10)
...
def synthetic_error():
    filename = './light_curves_files/VLT_680.csv' #nom du fichier d'entrée, c'est à dire de la courbe de transit
    ...
    df_lc.to_csv('./light_curves_files/VLT_680_bis.csv',sep=',',index=None)
